...
void TcpConn::SendPackData(const int32_t& mid,const char * buff, const int32_t & size)
{
	...
	m_socket.async_send(as::buffer(buf->buf, buf->use), [buf,this, self](as::error_code ec, std::size_t) {
		if (ec)
		{
			DoClose();
		}
...
bool TcpConn::DoReadHead()
{
	...
	auto self(shared_from_this());
	m_recvBuff.MakeRoome(m_msghead.size);
	as::async_read(m_socket, as::buffer(m_recvBuff.buf, m_msghead.size),
		[this, self](as::error_code ec, std::size_t recvsize) {
		if (!ec)
		{
			if (DoReadBody())
