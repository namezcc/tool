...
{\
    m_enable = false;\
	LOG_ERROR("connection_error Function %s Line %d %s\n", function, line,er.what());\
    return false;\
}\
catch(redis::timeout_error& er)\
{\
    m_enable = false;\
    LOG_ERROR("timeout_error Function %s Line %d %s\n", function, line,er.what());\
    return false;\
}\
catch(redis::protocol_error& er)\
{\
    LOG_ERROR("protocol_error Function %s Line %d %s\n", function, line,er.what());\
    return false;\
}\
catch(redis::key_error& er)\
{\
    LOG_ERROR("key_error Function %s Line %d %s\n", function, line,er.what());\
    return false;\
}\
catch(redis::value_error& er)\
{\
    LOG_ERROR("value_error Function %s Line %d %s\n", function, line,er.what());\
    return false;\
}\
catch (std::exception& er)\
{\
LOG_ERROR("exception Function %s Line %d - %s\n", function, line,er.what());\
return false; \
}
...
{
	if (Reconnect())
		LOG_INFO("Connect redis %s:%d success\n", m_host.c_str(), m_port);
	else
		LOG_ERROR("Connect redis %s:%d fail\n", m_host.c_str(), m_port);
}
...
bool RedisModule::Exists(const string& key)
{
	REDIS_CHECK;
	try
	{
...
bool RedisModule::HGetAll(const string & key, string_map& res)
{
	REDIS_CHECK;
	try
	{
